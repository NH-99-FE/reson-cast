...
export const playlistsRouter = createTRPCRouter({
  remove: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [deletePlaylist] = await db
        ...
        .returning()
      if (!deletePlaylist) throw new TRPCError({ code: 'NOT_FOUND' })
  ...
  removeVideo: protectedProcedure.input(z.object({ playlistId: z.uuid(), videoId: z.uuid() })).mutation(async ({ input, ctx }) => {
    ...
    if (!existingPlaylistVideo) throw new TRPCError({ code: 'NOT_FOUND', message: 'Video not in playlist' })
  ...
  addVideo: protectedProcedure.input(z.object({ playlistId: z.uuid(), videoId: z.uuid() })).mutation(async ({ input, ctx }) => {
    const { playlistId, videoId } = input
    const { id: userId } = ctx.user
    ...
    if (existingPlaylistVideo) throw new TRPCError({ code: 'CONFLICT', message: 'Video already in playlist' })
      ...
      const data = await db
        ...
        .where(
          and(
            ...
            cursor
              ? or(lt(playlists.updatedAt, cursor.updatedAt), and(eq(playlists.updatedAt, cursor.updatedAt), lt(playlists.id, cursor.id)))
              : undefined
          )
        )
